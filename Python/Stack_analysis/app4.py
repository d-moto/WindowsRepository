...
recommendation = ticker_data.recommendations
recommendation = recommendation.reset_index()
recommendation['Date'] = pd.to_datetime(recommendation['Date'])
recommendation = recommendation.sort_values(by='Date', ascending=False)
latest_recommendation = recommendation.iloc[0]['To Grade']
st.sidebar.write(f"最新の株式評価: {latest_recommendation}")
